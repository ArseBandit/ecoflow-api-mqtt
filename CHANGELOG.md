...
- **Delta 2 Max switch controls** — Separates Delta 2 and Delta 2 Max command
  maps so AC Output, X-Boost, AC Always On, and Beeper use the correct modules,
  state keys, and required command parameters on each device.

### Validation

- Verified the merged result with the full test suite: 17 tests passed.
- The switch behavior was tested by the contributor on real Delta 2 and Delta
  2 Max hardware.
